...
def main():
    glb_file_path = "results/P21_copy/fda_694_balance.glb"
    ...
    inhouse_file_name = "P21_quiet_stand_05"
    model_path = "results/P21_copy/lower_body.bioMod"
    c3d_file_path = f"/home/pariterre/Documents/ShareFolder/Felipe/backup_c3d/P21_copy/{inhouse_file_name}.c3d"
    kinematics_file_path = f"results/P21_copy/{inhouse_file_name}_q.npy"
    ...
    if trial_type == TrialType.SWAY:
        ...
        left_cycles = GaitCycle.extract_all(kinematics_data=inhouse, side=Side.LEFT, show_plot=False)
        if not left_cycles:
        ...
        right_cycles = GaitCycle.extract_all(kinematics_data=inhouse, side=Side.RIGHT, show_plot=False)
        if not right_cycles:
