...
def _get_kinematics_metrics(
    ...
    csv_file_path = _load_single_file(data_base_folder / "momentum_health_data" / subject, glb_filter, "csv")
    ...
    for subject in subject_names:
        ...
        if trial_type == TrialType.SWAY:
            ...
            if not all_cycles:
                ...
                print(
                    f"Cycles gait speed: {GaitCycle.mean_gait_speed_from_cycles(all_cycles, exclude_vertical=True):.2f} m/s"
                )
                print(
                    f"Cycles stride length: {GaitCycle.mean_stride_length_from_cycles(all_cycles, exclude_vertical=True):.2f} m"
                )
                print(f"Cycles stance time: {GaitCycle.mean_stance_time_from_cycles(all_cycles):.2f} s")
                print(f"Cycles swing time: {GaitCycle.mean_swing_time_from_cycles(all_cycles):.2f} s")
                print(
                    f"Cycles stride time: {GaitCycle.mean_stance_time_from_cycles(all_cycles) + GaitCycle.mean_swing_time_from_cycles(all_cycles):.2f} s"
                )
